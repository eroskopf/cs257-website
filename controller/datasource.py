...
def _prep_list(game_list):
    ...
    and statistics of each game, preserving the original order. The stats are divided 
    by 1000 so that the graph is easier to read.
...
def _save_graph(game1_list, game2_list):
    ...
    plt.ylabel('Players on Steam in Thousands', fontsize=12)
    plt.yticks(fontsize=8, rotation=90, va='center')
    plt.title('Popularity of ' + str(game1_list[0].get_title()) + " and " + str(game2_list[0].get_title()), fontsize=12)
    plt.subplots_adjust(left=0.15, bottom=0.2, right=0.9, top=0.9)
    plt.savefig('static/result_image.png', format='png')

def _save_one_line_graph(game_list):
    """
    a helper method that takes in one list of game objects and graphs the dates and stats. This graph
    is then saved into the static directory under the name result_image.png.
    
    Parameters:
        game_list: a list of game objects for the first game of interest
        
    Returns:
        nothing. it just saves the image 
    """
    game1 = _prep_list(game_list)
    plt.clf()
    plt.plot(game1[0], game1[1], color='purple')
    plt.xlabel('Time(Year-Month)', fontsize= 12)
    plt.xticks(fontsize=8, rotation=45, ha='right')
    plt.ylabel('Players on Steam in Thousands', fontsize=12)
    plt.yticks(fontsize=8, rotation=90, va='center')
    plt.title(str(game_list[0].get_title()) + '\'s Popularity')
    plt.subplots_adjust(left=0.15, bottom=0.2, right=0.9, top=0.9)
    plt.savefig('static/result_image.png', format='png')
...
class DataSource:
    ...
    def _get_number_of_avg_players_in_range(self, game_title, start, end):
        ...
        try:
            ...
            if len(return_list) < 1:
                #nothing got returned from the query
                query = "SELECT * FROM compareGames WHERE gameTitle = %s"
                cursor.execute(query, (game_title,))
                #check if the game is in the dataset at all
                return_list = cursor.fetchall()
                if len(return_list) >0:
                    #the game is in the dataset, just isn't out yet at this time
                    modified_list = []
                    #we add an empty game so the fill out data method will work as expected
                    empty_game = game.Game(game_title, end, 0)
                    modified_list.append(empty_game)
                    modified_list = _fill_out_data(modified_list, start, end)
    ...
    def list_all_top_games(self):
        """
        Returns a list of every game title that appears in our dataset, which is the top 100 most popular games on Steam atthe time this data was collected
       
       Return:
            a list of the top 100 most popular games on Steam at the time this data was collected, with no duplicates
        """
        try:
            cursor = self.connection.cursor()
            query = "SELECT gameTitle FROM topGames GROUP BY gameTitle ORDER BY gameTitle ASC"
    ...
    def list_popularity_over_all_time(self, game_title):
        ...
        with zeros when the game isn't in the dataset. 
        ...
        Return:
            ...
            It also graphs the data and saves this graph to the static directory. 
        """
        try:
            cursor = self.connection.cursor()
            query = "SELECT * FROM comparegames WHERE gametitle = %s"
            ...
            return_list = []
            _save_one_line_graph(returned_list)
